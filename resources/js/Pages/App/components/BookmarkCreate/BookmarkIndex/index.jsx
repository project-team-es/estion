...
export default function BookmarkIndex({ bookmarks }) {
    ...
    return (
        <div className="mt-8 bg-white overflow-hidden rounded-[12px] border p-8">
              <h2 className="text-2xl font-bold mb-6">登録されたURL一覧</h2>
            {bookmarks && bookmarks.length > 0 ? (
                <ul>
                    {bookmarks.map((bookmark) => (
                        <li key={bookmark.id} className="flex items-center justify-between py-3 ">
                            <div className="truncate">
                                <a href={bookmark.url} className="block py-1 text-blue-500 hover:text-blue-800 font-bold" target="_blank">{bookmark.name}</a>
                            </div>
                            <button
                                type="button"
                                onClick={() => handleDelete(bookmark.id)}
                                className="text-red-600 hover:text-red-800 focus:outline-none rounded-md ml-4"
                            >
                                削除
                            </button>
                        </li>
